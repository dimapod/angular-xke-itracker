...
describe('rating', function() {
  ...
  var compileHtml = function(html) {
    $rootElement.html(html ||
        '<btr-rating symbol="*" max="5" readonly="{{readonly}}" ' +
            'ng-model="$parent.model"></btr-rating>');
  ...
  it('should update the view on model', function() {
    ...
    expect($rootElement.find('li')).toHaveClass('btr-selected', 3);
  });
  ...
  it('should update the model on hover', function() {
    ...
    expect($rootElement.find('li')).toHaveClass('btr-hover', 3);

    ratingScope.leave(3-1);
    $rootScope.$apply();

    expect($rootElement.find('li')).toHaveClass('btr-hover', 0);
  });
  ...
  it('should ignore clicks when disabled', function() {
    ...
    expect($rootElement.find('li')).toHaveClass('btr-selected', 0);
  });
  ...
  it('should work inside ng-repeat', function() {
    compileHtml('<div ng-repeat="i in [1, 2, 3]"><btr-rating symbol="{{symbol}}" max="{{max}}" disabled="{{disabled}}" ng-model="$parent.i"></btr-rating></div>');

    expect($rootElement.find('div').eq(0).find('li')).toHaveClass('btr-selected', 1);
    expect($rootElement.find('div').eq(1).find('li')).toHaveClass('btr-selected', 2);
    expect($rootElement.find('div').eq(2).find('li')).toHaveClass('btr-selected', 3);
  });
});
