...
bugTrackerApp.directive('btrRating', function() {
  return {
    restrict: 'E',
    scope: {
      symbol: '@',
    ...
    link: function(scope, element, attrs, ngModel) {
      ...
      for(var i = 0; i < scope.max; i ++) {
        styles.push({ 'btr-selected': false, 'btr-hover': false });
      }

      scope.enter = function(index) {
        if (scope.readonly) return;
        angular.forEach(styles, function(style, i) {
          style['btr-hover'] = i <= index;
        });
      };
      ...
      function udpateSelectedStyles(index) {
        ...
        angular.forEach(styles, function(style, i) {
          style['btr-selected'] = i <= index;
        });
      }
    },
    template:
      '<ul class="btr-rating" ng-class="{\'btr-rating-pointer\':!readonly}">' +
